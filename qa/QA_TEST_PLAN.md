...
| T04 | SAVE_NOW 指令    | 群組輸入 `幫我記 週五要交報告`                            | Bot 回覆確認訊息 + Quick Reply         |      |      |
| T05 | CONFIRM 指令     | 點「確認」或輸入 `確認儲存記憶`                           | pending_actions 刪除，mem0 新增記憶    |      |      |
| T06 | CANCEL 指令      | `幫我記 測試取消`，再輸入 `取消儲存記憶`                  | pending_actions 刪除，mem0 無新增      |      |      |
| T07 | SAVE_PREVIOUS    | 先輸入一句，再輸入 `存上一則`                             | 回覆確認訊息，後續可存入記憶           |      |      |
| T08 | SAVE_QUOTED      | 回覆某句訊息並輸入 `幫我記`                               | 回覆確認訊息，後續可存入記憶           |      |      |
| T09 | QUERY 指令       | 輸入 `查 週五報告`                                        | 回覆答案 + 來源列表                    |      |      |
| T10 | HELP 指令        | 輸入 `help`                                               | 回覆幫助訊息                           |      |      |
| T11 | 1:1 不處理       | 私聊輸入 `幫我記 私聊`                                    | 無回覆、無寫入                         |      |      |
| T12 | Webhook 驗證     | LINE Console 點 Verify                                    | 驗證成功                               |      |      |
